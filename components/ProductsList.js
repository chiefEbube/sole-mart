    ...
    <div>
      <div className='inter flex items-center justify-center gap-5'>
        {
          products.map((product) => (
            <div className='bg-whiteBg mb-6 pb-5 w-[300px]'>
      ...
      <div className='flex justify-center gap-4 mt-14'>
        {pagination.map((page, i) => (
          <span className={`poppins text-[#667085] text-sm ${i !== 0 && i !== 8 ? 'font-semibold' : ''} py-2 px-4 border border-[#F3F3F3] ${i == 1 ? 'bg-[#F3EFF6]' : 'bg-[#bg-[#FAF7EF'}`}>{page}</span>
        ))}
      </div>
    </div>
  )
