    ...
    <div className="mb-16">
      <div className="grid sm:items-center grid-cols-1 sm:grid-cols-2 w-11/12 gap-4 mx-auto my-10">
        <div className="w-[300px] h-[300px] sm:h-[400px] lg:w-[514px] lg:h-[521px] mx-auto">
          <img src={`https://api.timbu.cloud/images/${product?.photos[0].url}`} alt={product.name} className="object-cover h-full w-full" />
        </div>
        ...
        <div className="flex flex-col justify-evenly lg:h-[514px] mt-4">
          ...
          <p className="text-xl font-bold text-[#eb2828d4] text-gray-800 flex items-center"><TbCurrencyNaira className="inline text-2xl" />{product?.current_price[0].NGN}</p>
          ...
          <div>
            <button onClick={() => addToCart(product.unique_id)} className="w-full lg:w-10/12 bg-secondaryBg text-plainWhite text-sm rounded-3xl inter font-semibold hover:opacity-90 duration-300 mt-12 sm:mt-8 py-4">Add to Cart
            </button>
          </div>
        </div>
      </div>
